...
1. Create a `config/` folder with a `config.yaml` file (see [Configuration](#configuration)):
...
3. Open <http://localhost:8156\> to view your jobs, trigger runs manually, and inspect logs.
...
Configure the `healthcheck` section to send HTTP callbacks at three phases: `start`, `end`, and `failure`. Each phase supports a URL, query params, and a JSON body. Use services like [Healthchecks](https://healthchecks.io) or [Uptime Kuma](https://github.com/louislam/uptime-kuma) to receive alerts. Alternatively, install `apprise` via the `software` list to push notifications from within a command.
