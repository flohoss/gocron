...
<template>
  <dialog :ref="(el) => emit('gotRef', el)" id="my_modal" class="modal modal-bottom sm:modal-middle">
    <form method="dialog" class="modal-box">
      ...
      <div class="py-4">
        <TextInput id="custom_command" title="Command" v-model="command" help="Example: restic snapshots"> </TextInput>
        <div class="flex justify-start flex-row-reverse gap-5">
          <button @click="emit('start', 'custom', command)" class="btn btn-primary"><i class="fa-solid fa-check"></i>Submit</button>
